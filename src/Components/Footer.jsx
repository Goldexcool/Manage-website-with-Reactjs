...
const Footer = () => {
  ...
  const getValue = (e) => {
    ...
    setUserInput(emailInput);
  };
  const btn = () => {
    setShowError(!showError);
    if (/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(userInput)) {
      setError("Sign up Sucessful");
    } else {
          ...
          <div className="input_btn">
            <div className="input-b">
              <input
                ...
                onChange={getValue}
              />
              {showError && <p className="errormess">{error}</p>}
              <p className="email_btn" onClick={btn}>
                Go
              </p>
            </div>
            <h1>Go Copyright 2020. All Rights Reserved</h1>
          </div>
        </div>
      </footer>
    </div>
